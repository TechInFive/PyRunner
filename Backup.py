...
class Player:
    ...
    def draw(self, screen):
        ...
        """
        # Draw arms and legs based on the run state for a walking cycle
        if self.run_state == 0:  # Contact position
            # Draw arms
            pygame.draw.line(screen, (0, 0, 0), arm_start, (arm_start[0] - arm_swing, arm_start[1]), 2)
            pygame.draw.line(screen, (0, 0, 0), arm_start, (arm_start[0] + arm_swing, arm_start[1]), 2)
            # Draw legs
            pygame.draw.line(screen, (0, 0, 0), leg_start, (leg_start[0] - LEG_LENGTH // 2, leg_start[1] + LEG_LENGTH), 2)
            pygame.draw.line(screen, (0, 0, 0), leg_start, (leg_start[0], leg_start[1] + LEG_LENGTH), 2)
        elif self.run_state == 1:  # Down position
            # Draw arms
            pygame.draw.line(screen, (0, 0, 0), arm_start, (arm_start[0], arm_start[1] + ARM_LENGTH), 2)
            pygame.draw.line(screen, (0, 0, 0), arm_start, (arm_start[0], arm_start[1] + ARM_LENGTH), 2)
            # Draw legs
            pygame.draw.line(screen, (0, 0, 0), leg_start, (leg_start[0], leg_start[1] + LEG_LENGTH), 2)
            pygame.draw.line(screen, (0, 0, 0), leg_start, (leg_start[0], leg_start[1] + LEG_LENGTH), 2)
        elif self.run_state == 2:  # Passing position
            # Draw arms
            pygame.draw.line(screen, (0, 0, 0), arm_start, (arm_start[0] + arm_swing, arm_start[1]), 2)
            pygame.draw.line(screen, (0, 0, 0), arm_start, (arm_start[0] - arm_swing, arm_start[1]), 2)
            # Draw legs
            pygame.draw.line(screen, (0, 0, 0), leg_start, (leg_start[0], leg_start[1] + LEG_LENGTH), 2)
            pygame.draw.line(screen, (0, 0, 0), leg_start, (leg_start[0] + LEG_LENGTH // 2, leg_start[1] + LEG_LENGTH), 2)
        elif self.run_state == 3:  # Up position
            # Draw arms
            pygame.draw.line(screen, (0, 0, 0), arm_start, (arm_start[0], arm_start[1] + ARM_LENGTH), 2)
            pygame.draw.line(screen, (0, 0, 0), arm_start, (arm_start[0], arm_start[1] + ARM_LENGTH), 2)
            # Draw legs
            pygame.draw.line(screen, (0, 0, 0), leg_start, (leg_start[0] - LEG_LENGTH // 2, leg_start[1] + LEG_LENGTH // 2), 2)
            pygame.draw.line(screen, (0, 0, 0), leg_start, (leg_start[0] + LEG_LENGTH // 2, leg_start[1] + LEG_LENGTH // 2), 2)
        """

        """
        # Draw legs based on run state for a detailed walking cycle
        # Draw arms and legs based on run state for a detailed walking cycle
        arm_start = body_start
        leg_start = body_end
        if self.run_state == 0:
            # Arms: Right arm forward, left arm back
            pygame.draw.line(screen, (0, 0, 0), arm_start, (arm_end[0] + ARM_LENGTH // 2, arm_end[1]), 2)
            pygame.draw.line(screen, (0, 0, 0), arm_start, (arm_end[0] - ARM_LENGTH // 2, arm_end[1]), 2)
            # Step 1: Left leg forward, right leg back
            # Left leg (forward and bent)
            pygame.draw.line(screen, (0, 0, 0), leg_start, (leg_start[0] - LEG_LENGTH // 4, leg_start[1] + LEG_LENGTH // 2), 2)
            pygame.draw.line(screen, (0, 0, 0), (leg_start[0] - LEG_LENGTH // 4, leg_start[1] + LEG_LENGTH // 2), (leg_start[0] - LEG_LENGTH // 8, leg_start[1] + LEG_LENGTH), 2)
            # Right leg (straight back)
            pygame.draw.line(screen, (0, 0, 0), leg_start, (leg_start[0] + LEG_LENGTH // 4, leg_start[1] + LEG_LENGTH), 2)
        elif self.run_state == 1:
            # Step 2: Transition
            # Arms and legs in neutral position (straight down)
            pygame.draw.line(screen, (0, 0, 0), arm_start, (arm_end[0], arm_end[1] + ARM_LENGTH), 2)
            pygame.draw.line(screen, (0, 0, 0), arm_start, (arm_end[0], arm_end[1] + ARM_LENGTH), 2)
            # Both legs straight down
            pygame.draw.line(screen, (0, 0, 0), leg_start, (leg_start[0], leg_start[1] + LEG_LENGTH), 2)
            pygame.draw.line(screen, (0, 0, 0), leg_start, (leg_start[0], leg_start[1] + LEG_LENGTH), 2)
        elif self.run_state == 2:
            # Step 3: Right leg forward, left leg back
            # Arms: Left arm forward, right arm back
            pygame.draw.line(screen, (0, 0, 0), arm_start, (arm_end[0] - ARM_LENGTH // 2, arm_end[1]), 2)
            pygame.draw.line(screen, (0, 0, 0), arm_start, (arm_end[0] + ARM_LENGTH // 2, arm_end[1]), 2)
            # Right leg (forward and bent)
            pygame.draw.line(screen, (0, 0, 0), leg_start, (leg_start[0] + LEG_LENGTH // 4, leg_start[1] + LEG_LENGTH // 2), 2)
            pygame.draw.line(screen, (0, 0, 0), (leg_start[0] + LEG_LENGTH // 4, leg_start[1] + LEG_LENGTH // 2), (leg_start[0] + LEG_LENGTH // 8, leg_start[1] + LEG_LENGTH), 2)
            # Left leg (straight back)
            pygame.draw.line(screen, (0, 0, 0), leg_start, (leg_start[0] - LEG_LENGTH // 4, leg_start[1] + LEG_LENGTH), 2)
        else:  # self.run_state == 3
            # Step 4: Transition back to Step 1
            # Arms and legs in neutral position (straight down) (same as state 1)
            pygame.draw.line(screen, (0, 0, 0), arm_start, (arm_end[0], arm_end[1] + ARM_LENGTH), 2)
            pygame.draw.line(screen, (0, 0, 0), arm_start, (arm_end[0], arm_end[1] + ARM_LENGTH), 2)
            # Both legs straight down (similar to Step 2)
            pygame.draw.line(screen, (0, 0, 0), leg_start, (leg_start[0], leg_start[1] + LEG_LENGTH), 2)
            pygame.draw.line(screen, (0, 0, 0), leg_start, (leg_start[0], leg_start[1] + LEG_LENGTH), 2)
"""
